...
(defun adjust-differential-geometry-lecture ()
  "Replaces cirtain symbols with equivelent but clearer ones in the notes and removes usage of \emph and the like that just add noise"
  (interactive)
  (save-excursion
    (replace-all-in-buffer-regexp "\\\\circ\\b" "\\circle" t)
    (replace-all-in-buffer-regexp "\\\\cap\\b" "\\intersection" t)
    (replace-all-in-buffer "\\bigcup" "\\bigunion")
    (replace-all-in-buffer "\\cup" "\\union")
    (replace-all-in-buffer "\\cong" "\\congruent")
    (replace-all-in-buffer "\\;" " ")
    (replace-all-in-buffer-regexp "\\\\\\([a-z]\\)\\b" "\\1" nil)
    (replace-all-in-buffer "\\var" "\\")
    (replace-all-in-buffer "\\quad" " ")
    (replace-all-in-buffer "\\leq" "<=")
    (replace-all-in-buffer "\\geq" ">=")
    (replace-all-in-buffer "\\hfill" "")
    (replace-all-in-buffer-regexp "\\\\textcolor{\\([a-z]*\\)}" "\\\\\\1" nil)
    ;; (replace-all-in-buffer-regexp "\\\\\\(cos\\|sin\\|phi\\|theta\\|rho\\|pi\\|alpha\\|beta\\|gamma\\|delta\\)" " \\1" nil)
    ;; (while (search-forward-regexp "\\\\mathbf{\\([a-zA-Z_0-9]*\\)}" nil)
    ;; (replace-match "\\1"))
    (replace-all-in-buffer "\\mathbf" "\\bold")
    (replace-all-in-buffer-regexp "\\\\\\(emph\\|def\\|mindef\\){\\([a-z0-9A-Z_'\\$\\\\(\\\\) \\-]*\\)}" "\\2" nil)
    (replace-all-in-buffer "\\displaystyle" " ")
    (replace-all-in-buffer "\n\n" "\n" )
  ))

(defun adjust-topology-notes ()
  "Replaces cirtain symbols with equivelent but clearer ones in the notes"
  (interactive)
  (save-excursion
  (beginning-of-buffer)
  (while (search-forward-regexp "\\\\emph{\\([a-z0-9A-Z_ ]*\\)}" nil t)
    (replace-match "\\1"))
  (beginning-of-buffer)
  (while (search-forward-regexp "\\\\circ\\(?!le\\)" nil t)
    (replace-match "\\circle" nil t))
  (beginning-of-buffer)
  (while (search-forward "\\cap" nil t)
    (replace-match "\\intersection" nil t))
  (beginning-of-buffer)
  (while (search-forward "\\bigcup" nil t)
    (replace-match "\\bigunion" nil t))
  (beginning-of-buffer)
  (while (search-forward "\\cup" nil t)
    (replace-match "\\cup" nil t))
  (beginning-of-buffer)
  (while (search-forward "\\intersectiontion" nil t)
    (replace-match "\\caption" nil t))
  (beginning-of-buffer)
  (while (search-forward "\\cong" nil t)
    (replace-match "\\congruent" nil t))
  (beginning-of-buffer)
  (while (search-forward "\n\n" nil t)
    (replace-match "\n" nil t))
  (beginning-of-buffer)
